...
from megatron.tokenizer.bert_tokenization import (BasicTokenizer, FullTokenizer as BertTokenizer, whitespace_tokenize)
...
def squad_data(train_file, cache_dir, max_seq_length=512, doc_stride=128,\
    max_query_length=512, seed=1234, vocab_file=None, skip_cache=True):
    print('*'*40)
    print("{},{},{}".format(train_file, cache_dir, vocab_file))
    print('*'*40)
    ...
    tokenizer = BertTokenizer(vocab_file) # for bert large
    ...
    train_examples = read_squad_examples(
        input_file=train_file, is_training=True, version_2_with_negative=False)
    ...
    if cache_dir is None:
        cached_train_features_file = train_file + '_{0}_{1}_{2}'.format(
            str(max_seq_length), str(doc_stride),
            str(max_query_length))
    else:
        cached_train_features_file = cache_dir.strip('/') + '/' + train_file.split('/')[-1] + '_{0}_{1}_{2}'.format(
            str(max_seq_length), str(doc_stride),
            str(max_query_length))
    ...
    try:
        ...
        # if not skip_cache and is_main_process():
    ...
    for f in train_features:
        train_data.append({'input_ids':np.array(f.input_ids),\
        'segment_ids': np.array(f.segment_ids),\
        'start_position': np.array(f.start_position), \
        'end_position': np.array(f.end_position), \
        'input_mask': np.array(f.input_mask)})
        # print("start_position,shape{}----- {}".format(np.array(f.start_position).shape, np.array(f.start_position)))
    ...
    # all_start_positions = np.array([f.start_position for f in train_features])
    # all_end_positions = np.array([f.end_position for f in train_features])
    
    # for 
    # train_sample = {
    #     'text': all_input_ids,
    #     'types': all_segment_ids,
    #     'labels': all_start_positions,
    #     'loss_mask': all_end_positions,
    #     'padding_mask': all_input_mask}
    # return train_sample, train_sample, train_sample
    # train_data = TensorDataset(train_sample)
    return train_data, train_data, train_data
    
    
    #SQuAD input_ids, input_mask, segment_ids, start_positions, end_positions = batch
